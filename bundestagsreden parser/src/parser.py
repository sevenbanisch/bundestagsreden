...
def parser(legislaturperiode):
    ...
    #added option of choosing legislative period
    datadir = f"./data_{legislaturperiode}/speeches/"
    ...
    with open(f"./data_{legislaturperiode}/missing_metadata.json", 'r', encoding='utf-8') as json_file:
                    ...
                    for info in element.getchildren():
                        ...
                        if info.attrib == {'klasse': 'redner'}:
                            ...
                            for p in info.getchildren()[0].getchildren()[0].getchildren():
                                #hier sollten redner ids hinzugefügt werden können
                                if p.tag == 'fraktion':
                                    party += p.text
                                if p.tag == 'vorname':
                                    try:
    ...
    with open (f"./speeches_{legislaturperiode}.jsonl", "w", encoding = "utf-8") as f:
        for line in flatdict:
            json.dump(line, f, ensure_ascii=False)
            f.write("\n")

    print(f"Success! Saved parsed speeches to './speeches_{legislaturperiode}.jsonl'.")
